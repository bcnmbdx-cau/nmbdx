...
$spreadsheet = $reader->load('../upload/'.$_FILES["file"]["name"]); //载入excel表格
...
for ($row = 1; $row <= $highestRow && $row<=10; ++$row) {
    $DO = $DO.$worksheet->getCellByColumnAndRow(1, $row)->getValue().","; //DO
    $pH = $pH.$worksheet->getCellByColumnAndRow(2, $row)->getValue().","; //PH
    $temperature = $temperature.$worksheet->getCellByColumnAndRow(3, $row)->getValue().","; //temperature
